...
def typingWrite(text):
    for char in text:
        ...
        sys.stdout.flush()
        time.sleep(0.04)    # text delay of 0.04 seconds
...
# Clear output screen when game ends
def clearScreen():
    os.system("cls")
...
def General_Restart():
    ...
    if General_Restart in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
        start_adventure(playerName)  

    elif General_Restart in ["no", "n", "nah", "nope", "not really", "never"]:
        time.sleep(0.4)    # text delay of 0.4 seconds
        clearScreen()    # screen clears
        typingWrite("Game ended!")
        sys.exit()    # system ends

    else:
        typingWrite("Error! Answer again.\n")
        General_Restart()    # replays General_Restart typingInput
...
while True:
    ...
    if ready in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok", "ready"]:
        playerName = typingInput("Great! What is your name? ")
        typingWrite("Welcome, %s! Let the adventure begin!\n\n" % (playerName))
        time.sleep(0.4)    # text delay of 0.04 seconds
        break
        ...
        typingWrite("Error! Answer again.\n")
        ready    # replays ready typingInput
        

# Adventure begins
def start_adventure(playerName):
    global chosen_weapon    # stores player's chosen weapon throughout the script
    ...
    if armoury in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
        ...
        typingWrite("Error! Answer again.\n")
        start_adventure(playerName)    # replays start_adventure typingInput
...
def obstacle_1(playerName, chosen_weapon):
        ...
        if user_choice in ["W", "West"]:
            ...
            # Continues to Obstacle 2 function with chosen weapon
            obstacle_2(playerName, chosen_weapon)

        elif user_choice in ["S", "South"]:
            direction = "S"
            typingWrite(obstacle1[direction])
            # Continues to Obstacle 3 with chosen weapon
            ...
            if restart_1 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure typingInput
                ...
                typingWrite("Error! Answer again.\n")
                General_Restart()    # returns to General_Restart function
            ...
            if restart_2 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure function

            elif restart_2 in ["no", "n", "nah", "nope", "not really", "never"]:
                obstacle_1(playerName, chosen_weapon)    # replays Obstacle 1 typingInput

            else:
                typingWrite("Error! Answer again.\n")
                General_Restart()    # returns to General_Restart
...
def obstacle_2(playerName, chosen_weapon):
    ...
    while True:
        ...
        if chosen_option in ["option one", "option 1", "one", "1"]:
            ...
            if restart_3 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure function
...
def obstacle_3(playerName, chosen_weapon):
    ...
    time.sleep(0.4)    # text delay of 0.4 seconds
    typingWrite("As you approach, you suddenly spot a massive troll blocking your path. The troll asks, 'Who dares to cross my bridge?' " +
                "You steel yourself and respond.\n")
    ...
    while True:
        ...
        if 1 <= guess <= 3:
            if guess == troll_number:
                typingWrite("The troll nods in approval. 'Correct! You may pass. Safe travels, adventurer.'\nWith the troll satisfied, "+
                            "you successfully cross the bridge. Your adventure continues...\n\n")
                time.sleep(0.4)    # text delay of 0.4 seconds
                obstacle_4(playerName, chosen_weapon)    # Continues to Obstacle 4 with chosen weapon
            ...
            chosen_ending = random.choice(ending_options)    # random option for ending
            ...
            if restart_5 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure function
...
def Ending_2(playerName, chosen_weapon):
    if chosen_weapon == None:    # if no weapon chosen previously
        ...
        if restart_6 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
            start_adventure(playerName)    # returns to start_adventure function
            ...
            typingWrite("Error! Answer again. ")
            Ending_2(playerName, chosen_weapon)    # replays Ending 2 tyingInput
    ...
    if user_choice in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
        choices = "yes"
        ...
        typingWrite("The Knight slowly stands and then bows. \nYou smile, and follow them towards a beautiful waterfall. "+
                    "\n\nIn a cove behind the waterfall, a chest of gold glitters back at you. \nYou collect the treasure triumphantly, "+
                    "and return victoriously to the castle!\n\n")
        time.sleep(0.4)    # text delay of 0.4 seconds
        typingWrite("Congrats! You found the treasure! You are brave and noble, Knight %s." % (playerName))
        time.sleep(0.4)    # text delay of 0.4 seconds
        clearScreen()    # screen clears
        ...
        choices = "no"
        typingWrite(Ending2[choices])
        obstacle_5(playerName, chosen_weapon)    # continues to Obstacle 5 function with chosen weapon

    else:
        typingWrite("That isn't an option! Pick one of the options! ")
        Ending_2(playerName, chosen_weapon)    # replays Ending 2 typingInput

### Ending 3 - Fairy
def Ending_3(playerName, chosen_weapon):
    typingWrite("Suddenly a fairy appears!\n\n")
    time.sleep(0.4)    # text delay of 0.4 seconds
    typingWrite("In a flash of light, you are outside flying through the air! \nThe fairy tells you she has been watching you, "+
                "and was impressed by your noble tactics.\n")
    time.sleep(0.4)    # text delay of 0.4 seconds
...
def obstacle_4(playerName, chosen_weapon):
    ...
    if user_choice in actions4:    # user's choice must match actions4 dictionary
        typingWrite(actions4[user_choice])

        if user_choice == 'hide':
           obstacle_5(playerName, chosen_weapon)    # Continues to Obstacle 5 with chosen weapon

        elif user_choice == 'fight':
            Ending_2(playerName, chosen_weapon)    # Continues to Ending 2 

        elif user_choice == 'climb':
            restart_7 = typingInput("You have died! Do you want to restart from the castle? \n").lower()

            if restart_7 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure function
        ...
        obstacle_4(playerName, chosen_weapon)    # replays Obstacle 4 typingInput
...
def obstacle_5(playerName, chosen_weapon):
    ...
    if user_choice in actions5:    # user's choice must match actions5 dictionary
        typingWrite(actions5[user_choice])

        if user_choice == 'back away':
            Ending_3(playerName, chosen_weapon)    # Continues to Ending 3

        elif user_choice == 'run away':
            restart_8 = typingInput("You have died! Do you want to restart from the castle? \n").lower()

            if restart_8 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)     # returns to start_adventure function
            ...
            if restart_9 in ["yes", "y", "yeah", "yea", "yup", "yeap", "sure", "okay", "ok"]:
                start_adventure(playerName)    # returns to start_adventure function
        ...
        typingWrite("That isn't an option! Pick one of the 3 choices! ")
        obstacle_5(playerName, chosen_weapon)    # replays Obstacle 5 typingInput


### Start the adventure
start_adventure(playerName)    # initialise start_adventure function
